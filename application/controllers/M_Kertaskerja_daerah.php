...
class M_Kertaskerja_daerah extends CI_Controller {
    var $view_dir   = "admin/kertaskerja_daerah/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/kertaskerja_daerah/kertaskerja.js";
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/kertaskerja_daerah/kertaskerja_daerah.js?v=".now("Asia/Jakarta");
    ...
    function alldownload(){
        ...
        $link = FCPATH."attachments/kertaskerja/";
        ...
        $sql = "SELECT K.id,U.name , K.attachments 
                ...
                WHERE KK.id=? AND U.group=7";
        $bind = array($idmap);
        $list_data = $this->db->query($sql,$bind)->result_array();
        
        foreach ($list_data as $v) {
            $downl = $link . $v['attachments'];
    
            // var_dump($downl);
            $this->zip->read_file($downl);
        }
        // die;
        
        $this->zip->download('Kertas_kerja');
    }

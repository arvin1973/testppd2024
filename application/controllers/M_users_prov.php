...
class M_users_prov extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/management/users_prov.js?v=" . now("Asia/Jakarta");
    ...
    function g_pengguna_aktif()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT * FROM `tbl_user` WHERE tbl_user.group = 4 AND last_access IS NOT NULL AND tbl_user.active_flag = 'Y' AND userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt', 'tpi1', 'dustintpi', 'testprovinsi')";
                ...
                $sql_prov = "SELECT * FROM `tbl_user` WHERE tbl_user.group = 4 AND userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt', 'tpi1', 'dustintpi', 'testprovinsi')";
                $list_data_prov = $this->db->query($sql_prov);
                if (!$list_data_prov) {
                    $msg = $session->userid . " " . $this->router->fetch_class() . " : " . $this->db->error()["message"];
                    log_message("error", $msg);
                    throw new Exception("Invalid SQL!");
                ...
                $response = array(
                    ...
                    "total"     => count($list_data_prov->result()),
                );
                $this->output
    ...
    function rekap_user()
    {
        ...
        $sql_prov = "SELECT * FROM `tbl_user` WHERE tbl_user.group = 4 AND userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt', 'tpi1', 'dustintpi', 'testprovinsi', 'Testkabkota-pusat', 'admin')";
        $list_data_prov = $this->db->query($sql_prov);
        if (!$list_data_prov) {
            $msg = $session->userid . " " . $this->router->fetch_class() . " : " . $this->db->error()["message"];
            log_message("error", $msg);
        ...
        foreach ($list_data_prov->result() as $value) {
    ...
    function get_datatable()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT A.id, A.userid,A.`name`,A.email,A.`active_flag`,A.`group`,B.`groupid`,B.`name` groupname,A.satker, A.last_access, C.nama_provinsi
                        FROM tbl_user A
                        INNER JOIN `tbl_user_group` B ON A.`group`=B.`id`
                        INNER JOIN `provinsi` C ON A.satker = C.id
                        WHERE B.`id`='4'";
                ...
                if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
                    $sql .= " AND ( "
                        ...
                        . " OR C.`nama_provinsi` LIKE '%" . $requestData['search']['value'] . "%' "
                        . ")";
                }
                $list_data = $this->db->query($sql);
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $tambah = "INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('" . $userid . "', '" . $this->input->post("name") . "','" . $email . "', '" . md5("pancasila" . "monda") . "','4','" . $satker . "','Y','" . $this->session->userdata(SESSION_LOGIN)->userid . "','$current_date_time')";

...
class M_users_tpt extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/management/users_tpt.js?v=" . now("Asia/Jakarta");
                ...
                $str = $this->load->view($this->view_dir . "index_tpt", $data_page, TRUE);
    ...
    function g_pengguna_aktif()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT * FROM `tbl_user` WHERE tbl_user.group = 2 AND last_access IS NOT NULL AND tbl_user.active_flag = 'Y' AND userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt')";
                ...
                $sql_tpt = "SELECT * FROM `tbl_user` WHERE tbl_user.group = 2 AND userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt')";
                $list_data_tpt = $this->db->query($sql_tpt);
                if (!$list_data_tpt) {
                    $msg = $session->userid . " " . $this->router->fetch_class() . " : " . $this->db->error()["message"];
                    log_message("error", $msg);
                    throw new Exception("Invalid SQL!");
                ...
                $response = array(
                    ...
                    "total"     => count($list_data_tpt->result()),
                );
                $this->output
    ...
    function rekap_user()
    {
        ...
        $sql_kabkota = "SELECT * FROM `tbl_user` WHERE tbl_user.group = 2 AND userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt', 'tpi1', 'dustintpi', 'testprovinsi', 'Testkabkota-pusat', 'admin')";
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $tambah = "INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('" . $userid . "', '" . $this->input->post("name") . "','" . $email . "', '" . md5("pancasila" . "monda") . "','2','','Y','" . $this->session->userdata(SESSION_LOGIN)->userid . "','$current_date_time')";

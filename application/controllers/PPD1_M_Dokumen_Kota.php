...
class PPD1_M_Dokumen_Kota extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd1/PPD1_upload_dokumen/dokumen_kota.js?v=" . now("Asia/Jakarta");
    ...
    function g_bahan()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($kate_wlyh == "kab" || $kate_wlyh == "kot") {
                    //LIST Dokumen
                ...
                $no = 1;
                $lnk = base_url("attachments/kabkota/");
                $str_view = '';
    ...
    function d_bahan()
    {
        ...
        } elseif ($kate_wlyh == "kab" || $kate_wlyh == "kot") {
            /*
                     * check data KAB / KOTA - start
                     */
            $sql = "SELECT A.*
    ...
    function get_spesifikdokumen()
    {
        ...
        $sql = "SELECT P.id, P.nama, P.jndok, S.jumlah FROM tbl_jenis_doc AS P JOIN ( SELECT jenisid,COUNT(*) AS jumlah FROM `t_doc_kab` WHERE isactive='Y' AND judul like '%kota%' GROUP BY jenisid ) AS S ON P.id = S.jenisid";

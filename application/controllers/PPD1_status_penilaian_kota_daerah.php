...
class PPD1_status_penilaian_kota_daerah extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd1/statuspenilaian_daerah/statuspenilaian_kota_daerah.js?v=" . now("Asia/Jakarta");
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT t1.*, ROUND((COUNT(skor.id) / (SELECT COUNT(I.`id`) FROM r_mdl1_item I JOIN `r_mdl1_sub_indi` SI ON SI.`id`=I.`subindiid` AND SI.`isactive`='Y' SI.isprov IN ('ALL', 'KOTKAB', 'KOT') JOIN `r_mdl1_indi` MI ON MI.`id`=SI.`indiid` AND MI.`isactive`='Y') * 100), 2) AS persentase_penilaian,
                        ...
                        WHERE t1.userid NOT IN ('tpt', 'teamtpt', 'peppd01', 'novi', 'dit.peppd', 'testtpt') AND t1.nama_kabupaten LIKE 'Kota %' AND t1.group=7
                        GROUP BY t1.id
    ...
    function downloadAllNilai()
    {
        ...
        foreach ($list_data->result() as $index => $d) {
            ...
            $this->excel->getActiveSheet()->setCellValue('D5', "$namakota");
            $this->excel->getActiveSheet()->mergeCells('A2:B2');
            $this->excel->getActiveSheet()->mergeCells('A3:B3');
        ...
        $this->excel->getActiveSheet()->setCellValue('C3', $namakota);
        $totalsheet=5;
        ...
        header("Content-Type:application/vnd.ms-excel");
        header("Content-Disposition:attachment;filename = Rekap_Nilai_". $namakota . ".xls");
        header("Cache-Control:max-age=0");
        $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
        $objWriter->save("php://output");
    }
    ...
    function rekap_resume_tpt_by_user()
    {
        ...
        $sql = "SELECT user.userid AS iduser, user.name AS nama_user, resume_kabkota.mapid AS mapid, wil.idkabkot AS idkabkot, kab.id AS idkabupaten, kab.nama_kabupaten AS nama_kabupaten, aspek.id AS idaspek, aspek.nama AS nama_aspek, resume_kabkota.ksmplan AS kesimpulan, resume_kabkota.saran AS saran
                ...
                WHERE kab.nama_kabupaten LIKE 'Kota %' AND user.group=7

...
class PPD4_M_Kertaskerja_Kab_daerah extends CI_Controller {
    var $view_dir   = "ppd4/kertaskerja_daerah/";
    var $js_init    = "main";
    var $js_path    = "assets/js/ppd4/kertaskerja_daerah/kertaskerja.js";
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd4/kertaskerja_daerah/kertaskerja_kab.js?v=".now("Asia/Jakarta");
        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                $sql = "SELECT K.id mapid,K.id_kab, K.nama_kabupaten
                        ...
                        WHERE urutan='0' AND P.id=$satkercode";
            ...
            try {
                ...
                elseif ($kate_wlyh=="kab") {
                    $sql = "SELECT K.id,U.name , K.attachments 
                            ...
                            JOIN `tbl_user` U ON W.iduser = U.id
                            WHERE KK.id=? AND KK.urutan='0' AND U.group=7";
                    $bind = array($idmap);
                    $list_data = $this->db->query($sql,$bind);
                ...
                elseif ($kate_wlyh=="kota") {
                    $sql = "SELECT K.id,U.name , K.attachments 
                            ...
                            JOIN `provinsi` P ON KK.prov_id = P.id_kode
                            JOIN `tbl_user` U ON W.iduser = U.id
                            WHERE KK.id=? AND KK.urutan='1' AND U.group=7";
                }
                if(!$list_data){

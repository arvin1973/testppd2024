...
class PPD4_M_users_tpt_daerah extends CI_Controller {
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd4/PPD4_user/users_tpt_daerah.js?v=".now("Asia/Jakarta");
                ...
                //List kab/kota
                $sql="SELECT * FROM provinsi where id ='$satkercode' ORDER By id_kode ASC";
    ...
    function get_kabkot(){
        ...
        $idprov = $this->db->query("SELECT * FROM tbl_user WHERE id=$idusr")->row_array();
        $idprov = $idprov['satker'];
        
        $kabkot = $this->db->query("SELECT K.id, K.`id_kab`, K.`nama_kabupaten`, P.nama_provinsi  
                        FROM `kabupaten` K
                        LEFT JOIN `provinsi` P ON P.id_kode= K.prov_id
                        WHERE P.id=$idprov")->result_array();

        $encrypted_kabkot = array_map(function($item) {
            $item['id'] = encrypt_text($item['id']);
            return $item;
        }, $kabkot);

        // Return encrypted data as JSON
        echo json_encode($encrypted_kabkot);
    ...
    function get_datatable(){
        if($this->input->is_ajax_request()){
            try {
                ...
                $sql = "SELECT A.id, A.userid,A.`name`,A.email,A.`active_flag`,A.`group`,A.cr_by,B.`groupid`,B.`name` groupname,A.satker, A.last_access
                        FROM tbl_user A
                        INNER JOIN `tbl_user_group` B ON A.`group`=B.`id`
                        WHERE B.`id`='7' AND A.satker='$satkercode'";
        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                $tambah="INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('".$userid."', '".$this->input->post("name")."','".$email."', '".md5("pancasila"."monda")."','7','" . $satker . "','N','".$this->session->userdata(SESSION_LOGIN)->userid."','$current_date_time')";
        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                foreach ($list_wilayah->result() as $r_wil) {
                    ...
                    $content.="<tr style='background-color: #E9E9E9;' class='odd gradeX'>";
                ...
                if($list_kab->num_rows()==0)
                    $content_k = "<tr style='background-color: #E9E9E9;'><td colspan='4'>Data tidak ditemukan</td></tr>";

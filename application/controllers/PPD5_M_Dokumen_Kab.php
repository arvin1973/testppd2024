...
class PPD5_M_Dokumen_Kab extends CI_Controller
{
    var $view_dir   = "ppd5/PPD5_upload_dokumen/";
    var $js_init    = "main";
    var $js_path    = "";
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd5/PPD5_upload_dokumen/dokumen_kab.js?v=" . now("Asia/Jakarta");

                $userid = $this->session->userdata(SESSION_LOGIN)->userid;
                $satker = $this->session->userdata(SESSION_LOGIN)->satker;
                $idcomb = "PROV-KAB-KOTA-" . $userid . "-" . $satker;
    ...
    function g_bahan()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT P.* FROM `kabupaten` P WHERE P.id=?";
                $bind = array($satker);
                $list_data = $this->db->query($sql, $bind);
                if ($list_data->num_rows() == 0)
                    throw new Exception("Data Kabupaten/Kota tidak ditemukan!", 0);
                $nmkabkot     = $list_data->row()->nama_kabupaten; //nama provinsi
                ...
                foreach ($list_data->result() as $v) {
                    ...
                    if ($v->nmdok == 'Dokumen_Inovasi') {
                        if ($v->mapid != null) {
                            ...
                            $idcomb     = "KAB-" . $v->id . "-" . $v->nmdok;
                            $tmp = "data-id='$idcomb'   ";
                            $tmp .= " data-nmdok='" . $v->nmdok . "_" . $nmkabkot . "'";
                    ...
                    if ($v->tautan != '') {
                        ...
                        // $str_j.="<td class='text-center _colSatu textpointer _btnEdiDok' style='padding: 3px; padding-left: 10px; padding-right: 10px; vertical-align: inherit; white-space: inherit; border: 1px solid black' ".$tmp."><a class='btn btn-xs btn-outline-warning waves-light' title='Edit Dokumen'>Edit Dokumen <i class='fas fa-edit' style='margin-left: 5px;'></i></a></td>";
                        if ($v->nmdok != 'Dokumen_Inovasi') {
                            $str_j .= "<td class='text-center textpointer' style='padding: 3px; padding-left: 10px; padding-right: 10px; vertical-align: inherit; white-space: inherit;'><a class='btn btn-xs btn-outline-warning _btnEdiDok' " . $tmp . " title='Edit Dokumen'>Edit Dokumen <i class='fas fa-edit' style='margin-left: 5px;'></i></a>" . $hapus . "</td>";
                        } else {
                        ...
                        // $str_j.="<td class='text-center _colSatu textpointer _btnUplDok' style='padding: 3px; padding-left: 10px; padding-right: 10px; vertical-align: inherit; white-space: inherit; border: 1px solid black' ".$tmp."><a class='btn btn-xs btn-outline-info waves-light' title='Unggah Dokumen'>Unggah Dokumen <i class='fas fa-upload' style='margin-left: 5px;'></i></a></td>";
                        if ($v->nmdok != 'Dokumen_Inovasi') {
    ...
    function save_dokumen_kab()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $jenisid = $list_data1->row()->jenisid;

                //spesifik tag user
                if (($jenisid == '17') || ($jenisid == '18')) {
     ...
     */
                $sql = "SELECT D.id mapid, D.judul, D.tautan, D.cr_by, D.cr_dt
                                ...
                                JOIN `t_doc_kab_groupuser` G ON D.id = G.docid 
                                JOIN `tbl_user_group` U ON G.`groupid` = U.id 
                                WHERE D.kabid =? AND D.judul=? AND D.isactive = 'Y'";
                // WHERE D.kabid =? AND D.judul=? AND U.groupid=? AND D.isactive = 'Y'";
                //$sql = "SELECT id, judul FROM t_doc_kab WHERE kabid=? AND judul=? AND isactive = 'Y'";

 ...
 * date : 10 des 2020
*/
class PPD7_bahan_dukung_kab extends CI_Controller
{
    var $view_dir   = "ppd7/PPD7_bahan_dukung/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/bahan_dukung/ppd7.js";
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $this->js_init    = "main";
                $this->js_tedit    = "main";
                $this->js_path    = "assets/js/ppd7/PPD7_bahan_dukung/bahan_dukung_kab.js?v=" . now("Asia/Jakarta");


                $data_page = array();
                $str = $this->load->view($this->view_dir . "index_ppd7_kab", $data_page, TRUE);
    ...
    function g_wilayah()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //table wilayah
                $sql = "
                        SELECT 'kab' kate, K.`id` mapid,K.`id_kab` kode,K.`nama_kabupaten` nmprov
                ...
                if ($list_data->num_rows() == 0) {
                    ...
                    foreach ($list_data->result() as $v) {
                        ...
                        $str .= "<img src='" . base_url() . "/assets/icons/PNG_KabupatenKota/" . $v->kode . "_" . $v->nmprov . ".png' alt='" . $v->nmprov . "' title='" . $v->nmprov . "' width='100' height='100' style='padding: 15px;'>";
            ...
            try {
                ...
                if ($kate_wlyh == "prov") {
                    ...
                    $sql = "SELECT D.`id` mapid, D.judul, D.filename
                            ...
                            WHERE  D.provid = '-1' OR D.provid =?  AND D.isactive = 'Y' ";
                    $bind = array($idmap);
                    $list_data = $this->db->query($sql, $bind);
                }
                if ($kate_wlyh == "kab") {
                    ...
                    $sql = "
                                SELECT 2 AS `no`,D.id mapid, D.judul, D.tautan, D.cr_by, D.cr_dt
                                    ...
                                    WHERE D.kabid =? AND D.isactive = 'Y' AND G.groupid =? ";
    ...
    function d_bahan()
    {
        ...
        if ($kate_wlyh == "PROV") {
            ...
            $filename = "dokumen_daerah_I_" . $name . ".zip";
            /*
                     * check data PROV - end
                     */
